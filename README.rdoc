...
=== Frontend
The frontend uses Backbone.js to organize its functions. Same as the backend, it has two types of collections, FilmList and FilmLocationList. These collections have their url pointing to the corresponding backend access points so Backbone.js will retrieve their objects.
